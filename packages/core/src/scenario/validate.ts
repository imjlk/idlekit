...
const SESSION_PATTERN_IDS = new Set([
  "always-on",
  "short-bursts",
  "twice-daily",
  "offline-heavy",
...
function validateEngineMetadata(issues: StandardIssue[], input: Record<string, unknown>): void {
  const engine = input.engine;
  if (engine === undefined) return;
  if (!isRecord(engine)) {
    pushIssue(issues, "engine must be an object when provided", "engine", engine);
    return;
  }
  if (typeof engine.name !== "string" || engine.name.trim().length === 0) {
    pushIssue(issues, "engine.name must be a non-empty string when provided", "engine.name", engine.name);
  }
  if (engine.version !== undefined && typeof engine.version !== "string") {
    pushIssue(issues, "engine.version must be a string when provided", "engine.version", engine.version);
  }
}

function validateUnitBlock(issues: StandardIssue[], input: Record<string, unknown>): void {
...
function validatePolicyBlock(issues: StandardIssue[], input: Record<string, unknown>): void {
...
function validateModelBlock(issues: StandardIssue[], input: Record<string, unknown>): void {
...
function validateSimEventLog(issues: StandardIssue[], eventLog: Record<string, unknown>): void {
...
function validateSimOfflineDecay(issues: StandardIssue[], decay: Record<string, unknown>): void {
...
function validateSimOffline(issues: StandardIssue[], offline: Record<string, unknown>): void {
...
function validateSimBlock(issues: StandardIssue[], input: Record<string, unknown>): void {
  ...
  if (sim.eventLog !== undefined) {
    ...
    if (!isRecord(eventLog)) {
      ...
      validateSimEventLog(issues, eventLog);
    }
  }
  ...
  if (sim.offline !== undefined) {
    ...
    if (!isRecord(offline)) {
      ...
      validateSimOffline(issues, offline);
    }
  }
}

function validateDesignBlock(issues: StandardIssue[], input: Record<string, unknown>): void {
  ...
  if (design.intent !== undefined && !DESIGN_INTENTS.has(String(design.intent))) {
    pushIssue(
      issues,
  ...
  if (design.sessionPattern !== undefined) {
    ...
    if (!SESSION_PATTERN_IDS.has(String(sessionPattern.id))) {
      pushIssue(
        issues,
    ...
    if (
      ...
      (typeof sessionPattern.days !== "number" || !Number.isInteger(sessionPattern.days) || sessionPattern.days <= 0)
    ) {
      pushIssue(
        issues,
...
function validateMonetizationRetention(issues: StandardIssue[], retention: Record<string, unknown>): void {
  ...
  if (d30 !== undefined) validateRate01(issues, d30, "monetization.retention.d30", "monetization.retention.d30");
  if (d90 !== undefined) validateRate01(issues, d90, "monetization.retention.d90", "monetization.retention.d90");
  if (
    isFiniteNumber(d1) &&
    isFiniteNumber(d7) &&
    isFiniteNumber(d30) &&
...
function validateMonetizationRevenue(issues: StandardIssue[], revenue: Record<string, unknown>): void {
...
function validateMonetizationUncertainty(issues: StandardIssue[], uncertainty: Record<string, unknown>): void {
  ...
  if (
    uncertainty.draws !== undefined &&
    (typeof uncertainty.draws !== "number" || !Number.isInteger(uncertainty.draws) || uncertainty.draws <= 0)
  ) {
    pushIssue(
      issues,
...
function validateMonetizationBlock(issues: StandardIssue[], input: Record<string, unknown>): void {
  const monetization = input.monetization;
  if (monetization === undefined) return;
  ...
  if (monetization.cohorts !== undefined) {
    ...
    if (!isRecord(cohorts)) {
      ...
      if (typeof cohorts.baseUsers !== "number" || !Number.isInteger(cohorts.baseUsers) || cohorts.baseUsers <= 0) {
        pushIssue(
          issues,
  ...
  if (monetization.retention !== undefined) {
    ...
    if (!isRecord(retention)) {
      ...
      validateMonetizationRetention(issues, retention);
    }
  }
  ...
  if (monetization.revenue !== undefined) {
    ...
    if (!isRecord(revenue)) {
      ...
      validateMonetizationRevenue(issues, revenue);
    }
  }
  ...
  if (monetization.uncertainty !== undefined) {
    ...
    if (!isRecord(uncertainty)) {
      ...
      validateMonetizationUncertainty(issues, uncertainty);
    }
  }
}

function validateAnalysisBlock(issues: StandardIssue[], input: Record<string, unknown>): void {
  ...
  if (analysis.experience === undefined) return;
  ...
  if (experience.series !== undefined && experience.series !== "money" && experience.series !== "netWorth") {
    pushIssue(
      issues,
  ...
  if (
    ...
    (typeof experience.draws !== "number" || !Number.isInteger(experience.draws) || experience.draws <= 0)
  ) {
    pushIssue(
      issues,
...
function normalizeSchemaIssues(result: unknown): StandardIssue[] {
...
function validateBaseScenario(input: unknown): StandardResult<ScenarioV1> {
  ...
  validateEngineMetadata(issues, input);
  ...
  validateDesignBlock(issues, input);
  validateMonetizationBlock(issues, input);
  validateAnalysisBlock(issues, input);
